...
DEVELOPER_IDENTITY=$(security find-identity -v | grep "Developer ID Installer" | grep -m 1 -oE '"[^"]+"' | tr -d '"')
set -e
...
if [[ $(echo "$DEVELOPER_IDENTITY" | shasum -a 256 | awk '{print $1}') == "bcf85e972ad33f433c3e117043f0c18f9718392e1becb7a08621285e64e0d3da" ]]; then
...
cp "build/postinstall-transition-$VERSION.sh" "sign/transition/postinstall"
...
productsign --sign "$DEVELOPER_IDENTITY" "sign/transition-$VERSION.pkg" "pkg/transition-$VERSION.pkg"
cd "pkg"
